...
public class GetWeb {
    ...
    private final int intThreadNum = 1; // 线程数
    private String strHomePage = ""; // 主页地址
    private String myDomain; // 域名
    private final String fPath = "CSDN"; // 储存网页文件的目录名
    private final ArrayList<String> arrUrls = new ArrayList<String>(); // 存储未处理URL
    private final ArrayList<String> arrUrl = new ArrayList<String>(); // 存储所有URL供建立索引
    private final Hashtable<String, Integer> allUrls = new Hashtable<String, Integer>(); // 存储所有URL的网页号
    private final Hashtable<String, Integer> deepUrls = new Hashtable<String, Integer>(); // 存储所有URL深度
    ...
    public static void main(String[] args) {
        GetWeb gw = new GetWeb("https://www.baidu.com/");
        gw.getWebByHomePage();
    }
    ...
    public void getWebByHomePage() {
        ...
        while (true) {
            if (arrUrls.isEmpty() && Thread.activeCount() == 1) {
                ...
                System.out.println("爬取的URL总数 = " + (webSuccessed + webFailed));
                System.out.println("成功的URL总数: " + webSuccessed);
                System.out.println("失败的URL总数: " + webFailed);
    ...
    public String getDomain() {
        String reg = "(?<=https\\://[a-zA-Z0-9]{0,100}[.]{0,1})[^.\\s]*?\\.(com|cn|net|org|biz|info|cc|tv|edu)";
